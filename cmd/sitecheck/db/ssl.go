...
func SSLChecksBySlugSince(db *DB, slug, outpostSlug string, since time.Time) ([]SSLCheck, error) {
	...
	rows, err := db.Query(
		...
		FROM checks_ssl WHERE slug = ? AND outpost_slug = ? AND timestamp >= ? ORDER BY timestamp`, slug, outpostSlug, sinceStr,
	)
	if err != nil {
