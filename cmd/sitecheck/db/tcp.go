...
func TCPChecksBySlugSince(db *DB, slug, outpostSlug string, since time.Time) ([]TCPCheck, error) {
	sinceStr := since.UTC().Format("2006-01-02 15:04:05")
	rows, err := db.Query(
		`SELECT id, slug, timestamp, duration_ms, pass, response_time_ms,
			host, port, remote_ip, error
		FROM checks_tcp WHERE slug = ? AND outpost_slug = ? AND timestamp >= ? ORDER BY timestamp`, slug, outpostSlug, sinceStr,
	)
	if err != nil {
