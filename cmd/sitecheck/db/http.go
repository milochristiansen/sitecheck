...
func InsertHTTPCheck(db *DB, c HTTPCheck) (int64, error) {
	result, err := db.Exec(
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		c.Slug, c.OutpostSlug, c.DurationMS, c.Pass, c.ResponseTimeMS, c.StatusCode, c.URL,
...
func HTTPChecksBySlugSince(db *DB, slug, outpostSlug string, since time.Time) ([]HTTPCheck, error) {
	...
	rows, err := db.Query(
		...
		FROM checks_http WHERE slug = ? AND outpost_slug = ? AND timestamp >= ? ORDER BY timestamp`, slug, outpostSlug, sinceStr,
	)
	if err != nil {
