...
// Generate renders the static site into cfg.OutputDir from the sorted slice of Results (each with History populated by
// the collector).
func Generate(cfg *Config, results []SiteResult) error {
	...
	for _, r := range results {
		if r.CheckType == "outpost" {
			outpostResults = append(outpostResults, r)
		} else {
