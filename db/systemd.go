...
// SystemdCheck holds a single row from checks_systemd.
...
func InsertSystemdCheck(db *DB, c SystemdCheck) (int64, error) {
	result, err := db.Exec(
		`INSERT INTO checks_systemd
			(slug, duration_ms, pass, response_time_ms, service_name, active_state, sub_state, load_state, main_pid, error)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func SystemdChecksBySlugSince(db *DB, slug string, since time.Time) ([]SystemdCheck, error) {
	...
	rows, err := db.Query(
		...
		FROM checks_systemd WHERE slug = ? AND timestamp >= ? ORDER BY timestamp`, slug, sinceStr,
	)
	if err != nil {
